...
world.results <- read.delim("world jam quotes raw.txt", 
                            sep="\n", quote="", header=F,
                            stringsAsFactors = F)
...
# NOT STANDARDIZED
focal.vars.noid <- c("focus.cos", "focus", "focus.sq", "log.length", "log.length.sq",  
...
quote.fit.10 <- cutQuoteModel(prev.thresh=0.1, controls)
summary(quote.fit.10)
# plot(fit.world.cut)


quote.fit.15 <- cutQuoteModel(prev.thresh=0.15, controls[-3])#, 
                              #filter.set = which(analysis.set$token.counts>=10))
...
quote.fit.15.noshort <- cutQuoteModel(prev.thresh=0.15, controls[-5], 
