var dateManagerTopics;
var topicLabels;
var wordFrequencies;
var topicDocs;
var docs;

function ready(error, dateManagerTopics_, topicLabels_, wordFrequencies_, topicDocs_, docs_) {

dateManagerTopics = dateManagerTopics_;
...
var managerValues = dateManagerTopics.filter(function (d) { 
  return d.manager == "Manager" && d.topicLabel == topicLabel.label; } );
var otherValues = dateManagerTopics.filter(function (d) { 
  return d.manager == "Other" && d.topicLabel == topicLabel.label; } );

var maxY = d3.max( [d3.max(managerValues, 
  function (d) { return d.high; }), d3.max(otherValues, function (d) { return d.high; })] );

var dates = managerValues.map(function (d) { return d.date; });
...
var yScale = d3.scale.linear().domain([0, maxY]).range([200 - padding, 5]);
	...
	timePlot.append("text").style("fill", "blue").attr("x", 450).attr("y", 15).text("Managers");
	timePlot.append("text").style("fill", "red").attr("x", 550).attr("y", 15).text("Others");

	timePlot.append("path").attr("d", area(managerValues.filter(function (d) { return d.date.substr(0, 1) == "1"; })))
	  .attr("class", "line")
	  .style("fill", "blue").style("opacity", "0.1");

	timePlot.append("path").attr("d", line(managerValues.filter(function (d) { return d.date.substr(0, 1) == "1"; })))
	  .attr("class", "line")
...
bottomRow.append("div").attr("class", "documents").append("div").attr("class", "docbox").selectAll(".document").data(selectedTopicDocs).enter()
  ...
  .text(function (d) { 
    var doc = docs[ d.doc ];
    return doc.id + ", " + doc.CreationDate + " [" + doc.manager + "]: " + doc.text;
  });
